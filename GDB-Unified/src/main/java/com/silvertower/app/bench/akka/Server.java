...
public class Server extends UntypedActor {
	...
	public void onReceive(Object message) throws Exception {
		...
		if (message instanceof BenchmarkConfiguration) {
			...
			GraphDescriptor gDesc = loadDB(f, d, currentInitializer);
			startRexsterServer();
			...
			getSender().tell(new AggregateResult(loadTimes), getSelf());
		}
	...
	public GraphDescriptor loadDB(File f, Dataset d, DBInitializer initializer) {
		...
		if (d.getNumberVertices() <= config.maxNVerticesMultiLoad) {
...
//		}
	...
	private List<Double> loadBenchmark(File datasetFile, DBInitializer initializer, String suffix, boolean batchLoading) {
		long before = System.nanoTime();
		int counter = 0;
	...
	public void loadGraphML(Graph g, InputStream is) {
		try {

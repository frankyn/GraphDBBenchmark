...
--------------------
To build GDB on a Linux host, run build.sh (ie. build.bat on a Windows host). This will create a GDBExec directory containing executable files.

Starting GDB
------------
Copy the GDBExec directory to as many machines as needed.
On each machine, you will have to modify the file config/application.conf. This file is the AKKA configuration file used to configure all the actors of the system.
...
To start the server on a Linux host, run startserver.sh (ie. startserver.bat on a Windows host).
To start the master client on a Linux host, run startmc.sh (ie. startmc.bat on a Windows host).
To start a slave client on a Linux host, run startsc.sh (ie. startsc.bat on a Windows host).
After having started the server, the master client and at least one slave client, the user is allowed to start running the benchmark by opening a command prompt, going to the GDBExec directory and typing:
